...
class Test_Plugin_Base extends \WP_UnitTestCase {
	...
	public function test_locate_plugin() {
		...
		$this->assertEquals( 'wp-middleware', $location['dir_basename'] );
		$this->assertContains( 'plugins/wp-middleware', $location['dir_path'] );
		$this->assertContains( 'plugins/wp-middleware', $location['dir_url'] );
	}
	...
	public function test_relative_path() {
		$this->assertEquals( 'plugins/wp-middleware', $this->plugin->relative_path( '/srv/www/wordpress-develop/src/wp-content/plugins/wp-middleware', 'wp-content', '/' ) );
		$this->assertEquals( 'themes/twentysixteen/plugins/wp-middleware', $this->plugin->relative_path( '/srv/www/wordpress-develop/src/wp-content/themes/twentysixteen/plugins/wp-middleware', 'wp-content', '/' ) );
	}
	...
	public function test_trigger_warning() {
		...
		set_error_handler( function ( $errno, $errstr ) use ( $obj ) {
			$obj->assertEquals( 'WPMiddleware\Plugin: Param is 0!', $errstr );
	...
	public function test_add_doc_hooks_error() {
		$mock = $this->getMockBuilder( 'WPMiddleware\Plugin' )
